...
- **视觉调用不缓存** — 相同附件在同一会话重复发送会重复调用视觉模型；缓存与去重是后续优化点。
- **主模型请求不保留图片** — 会话日志里仍保留原图片块，但主模型每次请求都被替换；若主模型需要"看图"之外的原图能力（如后续模型升级支持视觉），需停用本插件或调整 `provider`/`model`。
- **发现依赖 `llm/adapters-updated`** — 视觉模型在会话中途新增时，下一个 pre-step 会重扫；但已缓存的失败发现不会主动重试，除非拓扑变化或新会话。
