 ...
 * primary request.
 */
const DEFAULT_IMAGE_PROMPT = '请详细描述这张图片的内容，包括所有可见文字、数字、UI 元素、图表、品牌标识和布局结构。'
...
async function discoverVisionModel(ctx, config) {
    ...
    for (const provider of ctx.llm.listProviders()) {
        ...
        if (vision !== undefined)
            return { provider: provider.id, model: vision.id };
    }
    return undefined;
}
/**
...
export function apply(ctx, config) {
    ...
    ctx.on('llm/stream', (options, next) => {
        if (!isPrimaryRequest(config, options))
            return next();
